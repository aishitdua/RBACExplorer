...
export default function GraphTab({ slug }) {
  ...
  const selectedRoleData =
    selected?.type === 'role' ? rolesList.find((r) => r.id == selected.id) : null
              ...
              <div className="space-y-1.5">
                ...
                >
                  <option value="" disabled>
                    Choose a role to inherit from...
                  </option>
                  {rolesList
                    .filter(
                      (r) => r.id != selected.id && !selectedRoleData?.parents?.includes(r.id)
                    )
                    .map((r) => (
                ...
                <div className="flex flex-wrap gap-2">
                  {selectedRoleData.parents.map((pid) => {
                    ...
                    return (
                      <div
                        key={pid}
                        className="bg-gray-800 px-2 py-1 rounded text-[10px] text-gray-300 flex items-center gap-1"
                      >
                        {pname}
                        <button
                          onClick={async () => {
                            await removeParent(slug, selected.id, pid)
                            fetchData()
                          }}
