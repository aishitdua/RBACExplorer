...
async def analyze_project(slug: str, current_user: CurrentUser, session: DBSession):
    ...
    for row in result.fetchall():
        findings.append(
            ConflictFinding(
                ...
                detail={"permission_id": row.id, "permission_name": row.name},
            )
        )
    ...
    result = await session.execute(
        ...
        AND NOT EXISTS (
            SELECT 1 FROM role_inheritance ri WHERE ri.parent_role_id = r.id
        )
    """),
        {"pid": project.id},
    )
    for row in result.fetchall():
        findings.append(
            ConflictFinding(
                type="empty_role", detail={"role_id": row.id, "role_name": row.name}
            )
        )
...
async def diff_role(
    ...
    gained = [
        SimulatedResource(
            resource_id=rid,
            method=all_resources[rid].method,
            path=all_resources[rid].path,
            allowed=True,
        )
